...
function SpotsById() {
  ...
  const checkReviewOwner = () => {
    if (sessionUser && reviews !== null) {
      if (spot.ownerId !== null && spot.ownerId === sessionUser.id)
        return false;
      ...
      <div className="wholePage">
        ...
        <div id="description-box">
          ...
          <div id="price-box">
            ...
            <p id="rating">⭐{checkNumReviews(spot.numReviews)}</p>
            <button id="reserve" onClick={displayText}>
              Reserve
            </button>
            <label id="reserve-alert" hidden={reserve ? "" : "hidden"}>
        ...
        <div id="review-container">
          <h3 id="numReviews">⭐{checkNumReviews(spot.numReviews)}</h3>
          <h3>{checkReviewOwner() ? <CreateReviewButton /> : null}</h3>
          <Reviews spotId={id} />
        </div>
        <hr></hr>
      </div>
